...
char *BFS_PATH(t_stack *stack, t_param *p, t_cell grid[][p->height])
{
    ...
    path_str = calloc(stack->capacity + 1, sizeof(char));
    if (!path_str)
        return NULL;
    ...
    {
        curr = &(stack->path_t[stack->first]);
        w = grid[curr->x][curr->y].walls & 0xF;
        stack->first++;
